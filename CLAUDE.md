...
- ✅ **定期票 × 課程免費入場期間重疊補償（雙向）**（`3.20.0`→`3.21.0`，commit 後端 `37265f6`＋`3329fce`；E2E 5/5＋4/4）：政策（2026-07-17）——定期票有效區間與課程學員免費入場期間重疊 → 票到期日延長 overlap 天數。新 `passOverlapService`：
  ...
  - ⚠️ **已知邊界（未做）**：續約延長段再與課程期間重疊不另補（同課程冪等擋）；定期票轉讓後新持有人重疊不自動套；課程退費取消後已套補償不回收。
  - 📋 **同日資料操作**：新竹「入門班 7-8月週一班」加 **Raissa、黃詠慈**（各 8 堂、免費入場 7/06~9/06 跟課程）；**蔡逸夔** 查無會員 → 建待認領（技巧班另有温詩妤待認領）。⚠️ 使用者寫「Raissa Renata」、系統名「Raissa」當同一人處理（待確認）。

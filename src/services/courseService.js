...
// 薄包裝：實際迴圈/錯誤隔離邏輯收斂在 notificationService.notifyGymManagers（2026-09-12）。
const notifyCourseManagers = ({ gymId, type, title, body, referenceId, link }) =>
  notifyGymManagers({ gymId, type, title, body, referenceId, referenceType: 'courseEnrollment', link });
...
// TTL 內的重複呼叫共用同一份結果，大幅降低短時間內大量重複載入的讀取量。單一 Railway
// instance 執行、無跨機器快取一致性疑慮；不做「寫入時清快取」（enrollment 異動點分散在
// 十幾處呼叫端，逐一補寫入侵性高且容易漏），過期時間夠短、風險可接受。
// ⚠️ 2026-09-13 查 Firestore 查詢洞察資料：20 秒 TTL 上線後此查詢已從全站單日讀取次數最大宗
// （304,606／約29%）降至 47,567（約7%），但仍是次高——課程總覽名額顯示對「幾分鐘內的些微
// 落後」完全無感（同上，名額判定不靠這裡）。2026-09-13 使用者確認可接受、再放寬到 5 分鐘。
const _coursesCache = new Map(); // key: gymId || '__all__' → { data, expiresAt }
const COURSES_CACHE_TTL_MS = 300000;

...
- 🖨 **【P1+P2 硬體/軟體全數驗證完成，P3 上線範圍已改（2026-08-07）】發票機串接（WP-560 二聯式）**：完整設計見 `docs/invoice-integration-plan.md`。**P1（本地代理+列印+錢櫃）已完成並實機驗證**（`local-print-agent/`，見 2026-08-04 續10＋2026-08-06 續10 錢櫃測試）、**P2（發票號碼管理）已完成**（`invoiceNumberService.js`+`/invoices/state`，見 2026-08-04 續11）——通訊/中文列印/版面/對位裁切/號碼配發/開錢櫃皆已驗證，**無殘留硬體風險**。
  - ⚠️ **上線範圍改為「五流程一次到位」**（見設計文件 §6.1，2026-08-07 拍板，取代原「P3先接POS、P4再陸續接其餘」的漸進式做法）：使用者明確要求**同一場館內 POS/入場/課程/比賽報名/器材租借五個流程必須同時切換成真列印，不能只有部分流程上線**（避免同館現場記帳方式不一致）。上線節奏改成**按場館分批**（新竹先、士林晚幾天），靠**單一場館級開關**（`gyms/{gymId}.invoicePrintingEnabled` 類似設計）同時控制五項；既有 `InvoiceModal` 手動記帳版（課程/比賽/入場/POS 四處已上線）改成該開關 ON 時不渲染，不是刪掉重寫。**器材租借連手動版都還沒做**（`RentalsPage.jsx` 目前 0 引用 InvoiceModal），是五項裡最落後的一項。
  ...
  - **下一步**：先把器材租借補齊（跟其他四項一樣的資料結構）、`InvoiceCheckout`+`InvoicePrinter` adapter 五流程一起接、場館級開關、`local-print-agent` 正式部署到新竹櫃檯電腦——這幾項要一起備齊才能開新竹的開關。

...
- ✅ **續2（comp `87612d1`）**：①**贊助列移到比賽清單上方**（標題下方）——賽事多時不再被推到頁面深處，一進頁即可見 ②**清單排序改最新在最上**——順修既有缺陷：對接建立的賽事常無 `createdAt`、原排序會沉底（20260830 就因此排在舊賽事後面），改 createdAt 缺漏時用賽名開頭 YYYYMMDD 補當排序鍵。實機驗證兩者到位。③排列行為備忘：logo 列＝flex 自動換行置中，**不均分**（一排容量 10 時 12 個＝10+2、兩排各自置中；每排數量隨螢幕寬度自適應，桌機約 7~9 個、手機 2~3 個）。

...
> 後端 `/health` `3.400.0-schedule-event-group-experience`；E2E 3/3。三處白名單同步：`scheduleService.EVENT_CATEGORIES`＋`schedule.js` 兩個路由 validator＋前端 `SchedulePage.EVENT_CATEGORY_META`（👥 team teal `#0E7C86`，排在定線後/其他前）。循環安排/檢視彈窗/月曆色塊皆自動支援（讀同一 META）。

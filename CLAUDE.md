...
- ✅ **當期/隔期續報優惠＋續報截止日**（`3.63.0`→`3.64.0-renewal-deadline`，commit 後端 `d13ca81`＋`f22693f`、前端 `24e7701`；E2E 打正式 API **13/13**）：梯次加三欄——`currentTermRenewalDiscount`（當期續報優惠）/`prevTermRenewalDiscount`（隔期續報優惠，皆 NT$ 定額折抵）/`renewalDeadline`（**續報截止日，兩種優惠共用**、含當日、空＝不限）。
  - **後端權威計費**（enroll-all）：舊生狀態抽共用 `alumni{isCurrent,isPrev}`（同班別 categoryId 他梯有效報名 confirmed/leave 排除試上/補課；該梯 `endDate<今日`→isPrev、否則 isCurrent）——**開放日 gate 與續報優惠共用同一判定**；折抵順序＝插班比例價 → **當期優先折（其次隔期，不疊加）** → 隊員9折；enrollment 主筆存 `renewalDiscount`/`renewalDiscountType`（current_term|prev_term）稽核。截止日過期不折。
  - **前端**：員工加開/編輯梯次表單加三欄（兩個金額＋截止日期）；會員課程詳情琥珀橫幅擴充——報名開放公告之外，優惠設定且未過截止日時顯示「🎁 續報優惠：當期折 NT$X、上一期折 NT$Y（截止日止，系統自動折抵）」。
  - **E2E（13/13）**：當期學員 1000→**700**（折300、type current_term）／上期學員 →**800**（折200、prev_term）／新會員 1000 不折／**截止日改昨天後當期學員也不折**；enrollment 稽核欄全對；fixtures（班別/3梯/4會員/報名/轉帳單/交易）全清。

...
- 📋 **抱石墊/岩盔租借查證（2026-07-20，無異動）**：入場加購只有岩鞋/粉袋（checkIn 僅 shoesPrice/chalkPrice 欄），**無抱石墊/岩盔加購選項**；日結帳/月銷售「岩鞋(租借費)」列只含岩鞋+粉袋。器材租借 `/rentals`（多日+押金）品項 `systemSettings/rentalItems` **目前空**（未設抱石墊/岩盔）。營收報表「租借」欄＝入場岩鞋粉袋＋器材租借 rental* 交易。要讓抱石墊/岩盔進報表需先於器材租借加品項設費率。

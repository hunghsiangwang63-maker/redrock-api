...
- ✅ **DDoS 防護：Cloudflare 前置＋只認邊緣流量（2026-07-20 完成，全程營業中零中斷）**：網域 `redrocktaiwan.com` nameserver 已從 Porkbun 換到 **Cloudflare 免費版**（`graham.ns.cloudflare.com` / `journey.ns.cloudflare.com`），**營業中無縫切換**。
  ...
  - **🔑 密鑰位置（不進版控）**：`EDGE_SECRET` 存於 **Railway 環境變數** + **Cloudflare Transform Rule `inject-edge-auth`**（兩邊須一致）。輪替＝兩邊同步換新值。**⚠ Render 冷備尚未同步 `EDGE_SECRET`**——若故障轉移把 api CNAME 指 Render，Cloudflare 仍會注入 header 但 Render 無 `EDGE_SECRET` 會全擋 → **切 Render 前需先在 Render 補 `EDGE_SECRET`(+`EDGE_ENFORCE`)，或把 Render 的 EDGE_ENFORCE 設空**（動 Railway/Cloudflare 環境變數時一併同步 Render）。

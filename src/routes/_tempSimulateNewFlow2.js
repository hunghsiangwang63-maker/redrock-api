// ⚠️ 臨時診斷路由（比照本專案既有慣例）——修正版：課程場次清單從第一堂開始、計數排除已停課場次，
...
router.post('/course', authenticate, async (req, res) => {
  try {
    ...
    // 修正：整期全部場次（從第一堂開始，不受「今天」篩選），計數排除已停課場次（對齊 course.totalSessions）
    ...
    const validCount = allSessions.filter(s => !s.cancelled).length;
    ...
    const sessionsBlock = `
        <div style="background:#F7F3F3;border-radius:8px;padding:16px;margin:12px 0;font-size:13px;line-height:1.9;text-align:left">
          <div style="font-weight:600;color:#8B1A1A;margin-bottom:6px">課程場次（共 ${validCount} 堂，不含已停課場次）</div>
          ${sessionsHtml}
        </div>`;
    ...
    res.json({ ok: true, sentTo: demoEmail, cc: gymCc, validCount, totalSlots: allSessions.length, courseTotalSessions: course.totalSessions });

// ⚠️ 臨時診斷路由（比照本專案既有慣例）——僅供本次重新寄送 demo 合約信用（單一大文字框設定
// ＋ installmentPlanId 修復後版本確認），super_admin 限定，用完即從 index.js 移除並刪除此檔。
...
router.post('/course', authenticate, async (req, res) => {
  try {
    ...
    // 不特意帶 coursePlan——刻意讓 issueCourseContract 內建的自我修復 fallback（依 memberId+courseId
    // 反查 installmentPlans）接手，驗證這條路徑本身（非靠本路由自己查好再塞進去）確實正常運作。

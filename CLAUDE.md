...
- 🖨 **【待討論拍板】發票列印時機／方式**（2026-07-25 提出，記於 `invoice-integration-plan.md` 第 8 節）：三情境待決——**B** 一次來訪多筆消費（逐筆開票 vs 合併一張，建議逐筆）／**C** 非臨櫃付款（轉帳/LinePay 遠端、客人不在場）→ 建議「確認收款當下開票、收執聯放待領夾、到場再領」／**D** LinePay 入場→建議「一律到場掃碼確認當下印，付款當下不印；未入場轉券者持券來入場時才印」。方式（二聯同印/選填統編/補印不佔號/失敗佇列/現金才開櫃）多屬既定、一併確認。**下次討論決定 B/C/D 後補進「已定」清單並更新接線。**

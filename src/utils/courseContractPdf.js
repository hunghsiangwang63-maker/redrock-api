  ...
  return [
    {
      unbreakable: true,
      stack: [
        { text: '茲為甲方於乙方所提供教練服務事宜，經甲乙雙方同意依本契約履行，並同意上列條款，於下方簽名確認', margin: [0, 4, 0, 4], fontSize: 8.5 },
...
async function buildCourseContractPdfBuffer(data) {
  ...
  const docDefinition = {
    ...
    content: [
      ...
      { text: '紅石攀岩館 抱石課程服務同意書', bold: true, fontSize: 14, alignment: 'center', color: ACCENT, margin: [0, 0, 0, 2] },
      { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 531, y2: 0, lineWidth: 1.5, lineColor: ACCENT }], margin: [0, 0, 0, 4] },
      ...partiesBlock(data),
      ...courseContentBlock(data),
      ...paymentBlock(data),
      { text: '附註：本場館教練、團體課程，使用期限超過30天、總金額超過5000元，採按月逐月繳款，且付款期數不得低於契約期數(一期為一個月為限)。', fontSize: 7.5, color: '#666', italics: true, margin: [0, 0, 0, 1] },

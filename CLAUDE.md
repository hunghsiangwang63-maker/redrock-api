...
- 📋 **使用者已陸續提供的卡號區間**（供日後整理白名單參考，本次未使用於任何驗證邏輯）：黑卡 `AT19-0001~AT19-1000`／`AT21-0001~AT21-1502`／`ST19-0001~ST19-0021`；優惠卡 `D19-0001~D19-1000`／`D21-0001~D21-2000`／`D24-0001~D24-330`／`D24-1001~D24-1735`。**這些只是區間/字軌範例，還不是「已綁定 vs 可綁定」的最終分類清單**——等使用者整理好完整清單、明確指示要開始做白名單驗證，再實作「卡號需在可綁定清單內才能轉入/綁定」這一層（設計方向：清單存 Firestore 一個集合、以正規化後的卡號為 key、標記狀態 available/bound，bind 端點呼叫前先查該卡狀態）。
...
- **刪除範圍（判斷：只動她目前實際持有/可用的，已是歷史紀錄的不動）**：硬刪 2 筆 `status:active` 的入場券（一筆 online-entry 來源到期 2026-10-02、一筆 experience 來源已過期）＋硬刪那 1 筆黑卡；**保留**原本就 `cancelled` 的 2 筆入場券歷史紀錄未動。複查確認她名下入場券僅剩 2 筆（皆 cancelled）、黑卡 0 筆。

...
- ⚠️ **唯一未能親自走完的一段**：**沒有真實管理員/裁判密碼**（合理，不應嘗試取得/破解），無法用「正確密碼→拿到 token→實際成功寫入」這條快樂路徑做到完全的端到端點擊測試；改以「雜湊演算法逐位元核對一致」＋「標準、未經修改的 Firebase custom token 機制、沿用既有已證實可運作的服務帳號」＋「錯誤路徑全數正確拒絕」三者交叉佐證高度信心。**建議使用者用真實密碼手動走一次登入（管理員/裁判各一次）確認到位**，若有異常請回報。

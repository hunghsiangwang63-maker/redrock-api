...
router.get('/:id/waiver', authenticateAny, async (req, res) => {
  try {
    ...
    if (req.member && req.member.id !== memberId) {
      return res.status(403).json({ error: 'FORBIDDEN', message: '只能查看自己的聲明書' });
    }
    const db = getDb();
...
router.post('/:id/waiver/sign',
  ...
  async (req, res) => {
    try {
      ...
      const member = await memberService.getMember(memberId);
      const { signatureData, parentEmail, parentName, parentPhone, parentRelation } = req.body;

      if (member.isMinor && !parentEmail) {

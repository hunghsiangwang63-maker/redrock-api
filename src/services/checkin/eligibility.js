...
const getCourseAccess = async (memberId) => {
  ...
  for (const courseId of courseIds) {
    ...
    // 向下相容：舊課程若未設定此欄位，退而求其次用「開課日 + 入館有效天數」估算
    let practiceStart = course.unlimitedPracticeStart || course.startDate;
    const practiceEnd = course.unlimitedPracticeEnd ||
      (course.startDate ? dayjs(course.startDate).add(course.gymAccessDays != null ? Number(course.gymAccessDays) : 60, 'day').format('YYYY-MM-DD') : null);

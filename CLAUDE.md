...
- ✅ **員工課程月曆加課程篩選**（純前端 commit `e273a85`）：月份切換列下加下拉「全部課程／單一課程」（動態列出當月月曆內實際出現的課程、依名稱排序）——選定後**日格 pill 與點日場次詳情**都只顯示該課場次（`sessionsForDate` 加 courseId 過濾）。切月份保留篩選。附帶查證：會員端課程總覽梯次排序**本就正確**（7/9 `e4a24be`，正式資料模擬輸出 週一→週日+時段無誤；全課程 weekdays 欄位無缺漏），「學員端也要」經確認無需改動。

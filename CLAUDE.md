...
- 🔄 **DDoS 防護：Cloudflare 前置（2026-07-20，DNS 已搬、api 待翻橘雲）**：網域 `redrocktaiwan.com` nameserver 已從 Porkbun 換到 **Cloudflare 免費版**（`graham.ns.cloudflare.com` / `journey.ns.cloudflare.com`），**營業中無縫切換**。
  ...
  - **⬜ 剩餘步驟（Cloudflare active + Universal SSL 簽好後）**：① api 翻橘雲(Proxied)→ DDoS 防護啟動 ② Rules→Transform Rules 注入 `X-Edge-Auth: <secret>` request header ③ 實測 header 有到 Railway ④ Railway 設 `EDGE_ENFORCE='true'`+`EDGE_SECRET=<same>`+(選)`EDGE_HEADER`（3.80.0 中介層，堵直打原始網址）⑤ WAF/Under Attack Mode 位置。
  - **故障轉移更新**：DNS 現在 Cloudflare 管——切 Render 冷備＝在 **Cloudflare** 改 api CNAME 指 Render（非 Porkbun）；playbook `docs/outage-playbook.md` 待同步。
